...
$shift_types_icon = ["Crew" => "fa-truck-medical", "Event Crew" => "fa-house-medical-flag", "Medic" => "fa-car-side"];
...
<html lang="en">
  <head>
	...
	<title>Status-PVA | Pittsford Ambulance</title>
		...
		<div class="row">
			<div class="col-6">
                <div class="container-fluid bucket">
                    ...
                    <table class="table">
                        <?php // Populate crew data for On-Call Chief
                        if (array_key_exists($on_call_position, $status)) {
                            createCrewTable($status[$on_call_position], false);
                        } ?>
					</table>
				</div> <!-- end .bucket -->
			</div> <!-- end .col -->
		...
		<div class="row">
			<div id="duty-crew-wrapper" class="col-9">
				...
				<div class="vehicles container">
                    <?php // Iterate through current shifts and populate crew data for each shift
                    $row = 0;
                    foreach ($status as $shift_name=>$shifts) {
                        if (!in_array($shift_name, [$on_call_position, $supervisor_position])) {
                            $vehicle_icon = getShiftIcon($shift_types_icon, $shift_name);
                            displayCrewTable($shift_name, $shifts, $row, $vehicle_icon);
                            $row++;
                        }
                    } ?>
